-- Shared Savings V2: preserve economic contribution dates and support a safe
-- historical path by linking a pre-existing wallet adjustment (never mint a
-- member share without the matching personal-asset movement).

alter table public.shared_savings_requests
  add column if not exists contribution_date date,
  add column if not exists contribution_source_type text,
...
set contribution_date = created_at::date,
    contribution_source_type = 'wallet_contribution'
where request_type = 'contribution'
  and (contribution_date is null or contribution_source_type is null);

alter table public.shared_savings_requests
  add constraint shared_savings_contribution_source_valid check (
    request_type <> 'contribution'
    or (contribution_date is not null and contribution_source_type in ('wallet_contribution', 'linked_historical_movement'))
  );

create unique index if not exists shared_savings_requests_client_request_uidx
  on public.shared_savings_requests (requested_by_user_id, client_request_id)
  where client_request_id is not null;

-- A request is economically settled at most once. The index also safely
-- rejects concurrent approval retries atomically.
create unique index if not exists shared_savings_ledger_request_uidx
  on public.shared_savings_ledger (request_id);
...
begin
  ...
  if p_source_type not in ('wallet_contribution', 'linked_historical_movement') then raise exception 'Invalid contribution source type.'; end if;
  ...
  -- A retried client request returns its prior id without generating duplicate notifications.
  ...
  for v_approver in select a.user_id from public.shared_savings_approvers a join public.shared_savings_members m on m.shared_savings_id=a.shared_savings_id and m.user_id=a.user_id where a.shared_savings_id=p_shared_savings_id and m.status='active' loop
    insert into public.notifications (user_id,type,title,message,entity_type,entity_id,metadata) values
    (v_approver.user_id, 'shared_contribution_pending', 'Permintaan Setoran Masuk', v_user_name || ' mengajukan setoran ke "' || v_space.name || '".', 'shared_savings', p_shared_savings_id,
...
as $$
declare
  v_caller_id uuid := auth.uid(); v_request public.shared_savings_requests; v_space public.shared_savings;
  v_transaction public.transactions; v_transaction_id uuid; v_ledger_id uuid := gen_random_uuid(); v_total numeric; v_timezone text; v_economic_at timestamptz; v_new_balance numeric; v_member record;
begin
  ...
  insert into public.shared_savings_member_allocations (shared_savings_id,ledger_id,user_id,amount_signed)
  values (v_space.id,v_ledger_id,v_request.requested_by_user_id,v_request.amount);
  ...
  if v_space.target_amount is not null then
    ...
    if v_new_balance >= v_space.target_amount then
      ...
      if found then
        for v_member in select user_id from public.shared_savings_members where shared_savings_id=v_space.id and status='active' loop
          insert into public.notifications (user_id,type,title,message,entity_type,entity_id,metadata) values
