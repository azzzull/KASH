-- Derive the personal wallet server-side and authenticate based on managed space membership
...
begin
  ...
  select role into v_member_role from public.financial_space_members 
  ...
  insert into public.debt_payment_allocations (
    ...
    v_original_payer_id, v_personal_payment_id, v_personal_receivable.id, p_amount
  );
  ...
  insert into public.debt_payment_allocations (
    ...
    v_original_payer_id, v_managed_payment_id, v_managed_payable.id, p_amount
  );
