-- Fix record_cross_space_settlement
-- remaining_amount is derived by debt_progress_view, not stored in debts table
...
create or replace function public.record_cross_space_settlement(
    ...
    p_personal_wallet_id uuid, -- legacy parameter, ignored/overridden internally
...
begin
  ...
  if v_outstanding != coalesce(v_personal_receivable.remaining_amount, v_outstanding) or v_outstanding != coalesce(v_managed_payable.remaining_amount, v_outstanding) then
  ...
  insert into public.cross_space_settlements (
    ...
    v_user_id, p_event_id, p_amount, p_managed_wallet_id, v_actual_personal_wallet_id, p_settlement_date, p_client_request_id
  ...
  insert into public.debt_payment_allocations (
    ...
    v_user_id, v_personal_payment_id, v_personal_receivable.debt_id, p_amount
  );

  update public.debts set status = case when v_personal_receivable.remaining_amount - p_amount <= 0 then 'settled'::public.debt_status else 'partially_paid'::public.debt_status end where id = v_personal_receivable.debt_id;
  ...
  insert into public.debt_payment_allocations (
    ...
    v_user_id, v_managed_payment_id, v_managed_payable.debt_id, p_amount
  );

  update public.debts set status = case when v_managed_payable.remaining_amount - p_amount <= 0 then 'settled'::public.debt_status else 'partially_paid'::public.debt_status end where id = v_managed_payable.debt_id;
