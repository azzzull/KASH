-- 202608270016_personal_advance_to_managed.sql

-- 1. Update validation trigger with new matrix and role
...
begin
  ...
  if new.cross_space_event_id is not null then
    ...
    if new.cross_space_role = 'managed_spending' then
      ...
      -- Enforce role matrix
      if v_event.event_type not in ('managed_expense_paid_personally', 'personal_advance_to_managed') then
        raise exception 'invalid event type for personal_cash_out';
      end if;

    elsif new.cross_space_role = 'managed_advance_cash_in' then
      if new.wallet_id is null then raise exception 'managed_advance_cash_in must have wallet'; end if;
      if new.destination_wallet_id is not null then raise exception 'managed_advance_cash_in must have null destination_wallet_id'; end if;
      if new.type != 'adjustment' then raise exception 'managed_advance_cash_in must be adjustment'; end if;
      if new.space_id != v_event.managed_space_id then raise exception 'managed_advance_cash_in space mismatch'; end if;
      if v_space_type != 'managed' then raise exception 'managed_advance_cash_in space must be managed'; end if;
      if v_event.event_type != 'personal_advance_to_managed' then raise exception 'invalid event type for managed_advance_cash_in'; end if;
      if new.amount != v_event.amount then raise exception 'managed_advance_cash_in amount must match event amount'; end if;

    else
      raise exception 'invalid cross_space_role';
    end if;
  end if;
...
-- 2. Create record_cross_space_advance RPC
