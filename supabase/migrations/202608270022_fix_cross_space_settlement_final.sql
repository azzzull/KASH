...
begin
  ...
  update public.debts set status = case when v_personal_receivable.remaining_amount - p_amount <= 0 then 'settled' else 'partially_paid' end where id = v_personal_receivable.debt_id;
  ...
  update public.debts set status = case when v_managed_payable.remaining_amount - p_amount <= 0 then 'settled' else 'partially_paid' end where id = v_managed_payable.debt_id;
