...
-- 3. Create delete_goal_if_empty RPC
...
begin
  ...
  -- Delete the wallet first if it exists
  if v_goal_record.wallet_id is not null then
    delete from public.wallets where id = v_goal_record.wallet_id;
  end if;
...
-- 4. Update close_goal_with_sweep to handle already cancelled goals
