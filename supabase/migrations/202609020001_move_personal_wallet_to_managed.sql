-- KASH: Move an existing Personal wallet into a Managed Space.
-- The mutation RPC is intentionally conservative: it only auto-migrates
-- ordinary wallet-only ledger rows and blocks ambiguous domain dependencies.
...
declare
  ...
  v_review_transfers jsonb := '[]'::jsonb;
...
begin
  ...
  v_safe_dependencies := v_migratable_transactions + v_custom_categories;
  ...
  if jsonb_array_length(v_review_transfers) > 0 then
    v_blocking_issues := v_blocking_issues || jsonb_build_array(jsonb_build_object(
      'classification', 'BLOCK_AND_REVIEW',
      'code', 'personal_cross_wallet_transfers',
      'title', 'Historical transfers with Personal wallets require review.',
      'count', jsonb_array_length(v_review_transfers),
      'items', v_review_transfers
    ));
  end if;

  with blocking as (
    select 'transfers' as code, 'Transfers cannot be re-scoped automatically.' as title, count(*)::integer as count
    ...
    where (wallet_id = p_wallet_id or destination_wallet_id = p_wallet_id)
      and not (
        ...
        and space_id = v_personal_space.id
        and type in ('income', 'expense', 'adjustment')
        and envelope_id is null
        and related_entity_type is null
        and related_entity_id is null
        and cross_space_event_id is null
        and cross_space_role is null
      )
    union all
  ...
  if jsonb_array_length(v_review_transfers) > 0 then
    v_blocking_issues := jsonb_set(
      v_blocking_issues,
      array[(select ordinality - 1 from jsonb_array_elements(v_blocking_issues) with ordinality where value->>'code' = 'transfers' limit 1)::text, 'items'],
      v_review_transfers,
      false
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'can_move', v_blocking_count = 0 and v_migratable_transactions = v_total_transactions
  );
end;
$$;
