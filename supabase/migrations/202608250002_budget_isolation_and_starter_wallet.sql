-- ============================================================
-- Migration: 202608250002_budget_isolation_and_starter_wallet.sql
-- 1. Space-scoped Budget Progress and Overview RPCs
-- 2. Space-scoped Create Budget Target RPC
-- 3. Atomic Managed Space Starter Wallet Creation Trigger
-- ============================================================

-- Drop existing overloaded or differently named parameter functions
drop function if exists public.get_monthly_budget_progress(date);
drop function if exists public.get_monthly_budget_progress(date, uuid);
drop function if exists public.get_monthly_budget_progress(uuid, date);
drop function if exists public.get_monthly_budget_overview(date);
drop function if exists public.get_monthly_budget_overview(date, uuid);
drop function if exists public.get_monthly_budget_overview(uuid, date);
drop function if exists public.create_budget_target(text, text, numeric, date, boolean, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text);
drop function if exists public.create_budget_target(text, text, numeric, date, boolean, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text, uuid);

-- 1. Space-Scoped Monthly Budget Progress RPC
...
-- 2. Space-Scoped Monthly Budget Overview RPC
...
-- 3. Space-Scoped Create Budget Target RPC
...
-- 4. Atomic Managed Space Starter Wallet Creation Trigger
create or replace function public.financial_spaces_create_managed_starter_wallet()
returns trigger
language plpgsql
security definer
set search_path = public
...
create trigger financial_spaces_create_managed_starter_wallet_trigger
  after insert on public.financial_spaces
  for each row
  execute function public.financial_spaces_create_managed_starter_wallet();

grant execute on function public.get_monthly_budget_progress(date, uuid) to authenticated;
