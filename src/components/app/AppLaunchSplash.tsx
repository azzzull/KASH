...
function AppLaunchSplash({ isExiting }: { isExiting: boolean }) {
  return (
    <div
      aria-hidden="true"
      className={`fixed inset-0 z-[70] flex h-dvh min-h-[100dvh] w-screen touch-none select-none items-center justify-center overflow-hidden bg-white px-6 ${
        isExiting ? "kash-launch-splash--exit" : "kash-launch-splash--enter"
      }`}
    >
      <img
        alt=""
        className="kash-launch-splash__logo h-auto w-[clamp(200px,58vw,230px)] max-w-[calc(100vw-3rem)] md:w-[clamp(220px,20vw,260px)]"
        draggable={false}
