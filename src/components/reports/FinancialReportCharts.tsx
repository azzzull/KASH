...
import { buildCashFlowScale, buildCashFlowTrend, donutRingColor } from "../../lib/reportCharts";
...
function CategoryChart({ data, noData }: { data: TransactionRecapData; noData: string }) {
  const items = data.spendingBreakdown.filter((item) => item.amount > 0); if (!items.length) return <NoData text={noData} />;
