...
import kashLogo from "../../../logo/SVG/KASH Logo.svg";
...
export function AppHeader({ visible }: AppHeaderProps) {
  ...
  const initial = profile?.full_name?.charAt(0) ?? profile?.email.charAt(0) ?? "A";
  ...
  return (
    <header
      className={`sticky top-0 z-20 shrink-0 border-b border-kash-emerald/15 bg-white/90 backdrop-blur transition-[transform,margin] duration-200 lg:hidden ${
        visible ? "translate-y-0" : "pointer-events-none -mb-16 -translate-y-full"
      }`}
    >
      <div className="flex min-h-16 items-center justify-between gap-4 px-4">
        <Link to="/dashboard" aria-label="KASH Dashboard" className="inline-flex min-w-0 items-center">
          <img src={kashLogo} alt="KASH" className="h-7 w-auto" />
        </Link>

        <div className="flex items-center gap-3">
          <div ref={notificationsRef} className="relative">
            <IconButton icon={Bell} label="Open notifications" onClick={() => setNotificationsOpen((current) => !current)} />
            {unreadCount > 0 ? (
            ...
            {profileMenuOpen ? (
              <div className="absolute right-0 top-[calc(100%+8px)] z-40 w-56 rounded-lg border border-kash-emerald/15 bg-white p-2 shadow-soft">
                <div className="border-b border-slate-100 px-3 py-2">
                  <p className="truncate text-sm font-extrabold text-slate-900">{profile?.full_name || "Account"}</p>
                  <p className="truncate text-xs font-semibold text-slate-600">{profile?.email}</p>
                </div>
                <button
                  type="button"
                  onClick={() => {
