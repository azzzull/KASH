...
import type { BudgetVsActualItem, FinancialHealthReportData, FinancialReportData, TransactionRecapData } from "../types/reports";
...
import { budgetPerformanceStatusLabel } from "./budgetPerformance";
...
function normalizeFinancialReport(data: FinancialReportData): FinancialReportData {
  ...
  return {
    ...
    transactionRecap: {
      ...
      spendingBreakdown: (recap.spendingBreakdown ?? []).map((item) => ({ ...item, groupName: item.groupName || (item.groupType === "envelope" ? "Unknown Envelope" : "Uncategorized"), amount: safeNumber(item.amount), transactionCount: safeNumber(item.transactionCount), percentage: safeNumber(item.percentage) })),
...
function transactionDescription(transaction: TransactionRecapData["transactions"][number]) { return displayTitle(transaction); }
function drawTransferDirection(doc: jsPDF, transaction: TransactionRecapData["transactions"][number], x: number, y: number, width: number) {
  const from = transaction.wallet?.name ?? "-"; const to = transaction.destinationWallet?.name ?? "External";
  doc.setCharSpace(0); doc.setFont("helvetica", "normal"); doc.setFontSize(6.3); doc.setTextColor(71, 85, 105);
  const safeFrom = clipped(doc, from, Math.max(18, width * 0.42)); const safeTo = clipped(doc, to, Math.max(18, width * 0.42));
  doc.text(safeFrom, x, y); const start = x + doc.getTextWidth(safeFrom) + 2; const end = Math.min(x + width - doc.getTextWidth(safeTo) - 3, start + 12);
  if (end > start + 3) { doc.setDrawColor(100, 116, 139); doc.setLineWidth(0.25); doc.line(start, y - 0.8, end, y - 0.8); doc.setFillColor(100, 116, 139); doc.triangle(end, y - 0.8, end - 2, y - 2, end - 2, y + 0.4, "F"); }
  doc.text(safeTo, Math.max(end + 2, start + 4), y);
}
...
function drawFinancialHealth(doc: jsPDF, data: FinancialReportData) {
  const source = data.financialHealth; if (!source) return;
  const health: FinancialHealthReportData = { ...source, budgets: [] };
...
function drawBudgetVsActual(doc: jsPDF, data: FinancialReportData) {
  ...
  if (data.unbudgetedSpending?.length) { sectionTitle(doc, "Outside Budget", y); y += 5; doc.setFont("helvetica", "normal"); doc.setTextColor(71, 85, 105); doc.setFontSize(7); data.unbudgetedSpending.forEach((item) => { doc.text(`${item.categoryName} · ${item.transactionCount} transaction${item.transactionCount === 1 ? "" : "s"}`, PAGE.left, y); doc.text(formatCurrency(item.amount), PAGE.right, y, { align: "right" }); y += 5; }); y += 4; }
  if (data.planningInsights?.length) { sectionTitle(doc, "Monthly Planning Insights", y); y += 5; doc.setTextColor(71, 85, 105); doc.setFontSize(6.8); data.planningInsights.slice(0, 5).forEach((item) => { const lines = doc.splitTextToSize(`${item.message} ${item.suggestedAction}`, 172); if (y + lines.length * 4 + 4 > 276) return; doc.setFont("helvetica", "bold"); doc.text(item.title, PAGE.left, y); y += 3.5; doc.setFont("helvetica", "normal"); doc.text(lines, PAGE.left, y); y += lines.length * 4 + 3; }); }
}
...
async function drawCategoryRingChart(doc: jsPDF, title: string, items: { label: string; value: number; percentage?: number }[], x: number, y: number, width: number, height: number) {
  ...
  const segments = categories.map((item, index) => { const dash = item.value / total * availableDegrees / 360 * circumference; const segmentOffset = offset; offset += dash + gapDegrees / 360 * circumference; return { ...item, color: getDonutChartColor(index), dash, offset: segmentOffset }; });
...
export async function exportTransactionRecapPdf(data: TransactionRecapData) {
  ...
  data.transactions.forEach((transaction, index) => {
    ...
    doc.setCharSpace(0); doc.setTextColor(30, 41, 59); doc.setFont("helvetica", "normal"); doc.setFontSize(6.7); cells.forEach((cell, cellIndex) => { const align = cellIndex >= 4 ? "right" : "left"; const x = align === "right" ? positions[cellIndex] + widths[cellIndex] - 1 : positions[cellIndex] + 2; doc.text(clipped(doc, cell, widths[cellIndex] - 2), x, y + 4.5, { align }); });
    if (transaction.type === "transfer") drawTransferDirection(doc, transaction, positions[1] + 2, y + 9, widths[1] - 3);
...
export async function createFinancialReportPdfBlob(rawData: FinancialReportData): Promise<Blob> {
  ...
  try { await drawCategoryRingChart(doc, l.category, recap.spendingBreakdown.map((item) => ({ label: item.groupName, value: item.amount, percentage: item.percentage })), PAGE.left, 131, 178, 75); } catch (error) { console.error("[KASH Financial Report Export][chart-category]", error); drawRankedChart(doc, l.category, recap.spendingBreakdown.map((item) => ({ label: item.groupName, value: item.amount, percentage: item.percentage })), PAGE.left, 131, 178, 75); }
  ...
  try {
    ...
    doc.text("Includes spending inside Envelopes.", PAGE.left, 25);
    drawRankedChart(doc, "Category Analysis", recap.categoryBreakdown.map((item) => ({ label: item.categoryName, value: item.amount, percentage: item.percentage })), PAGE.left, 31, 178, 70);
    doc.setFont("helvetica", "normal"); doc.setTextColor(100, 116, 139); doc.setFontSize(6.5);
    doc.text(`Total category spending: ${formatCurrency(recap.categoryBreakdown.reduce((sum, item) => sum + item.amount, 0))}`, PAGE.left, 108);
