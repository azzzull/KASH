...
export async function createFinancialReportPdfBlob(rawData: FinancialReportData): Promise<Blob> {
  ...
  try {
    ...
    y += 45; sectionTitle(doc, "Largest Transactions", y); y += 5;
    ...
    const topTransactions = [...recap.transactions].sort((a, b) => Math.abs(transactionTotal(b)) - Math.abs(transactionTotal(a))).slice(0, 8); y = drawTransactionHeader(y);
