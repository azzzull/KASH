...
function progressBar(doc: jsPDF, x: number, y: number, width: number, percent: number, color: readonly [number, number, number] = EMERALD) { doc.setFillColor(226, 232, 240); doc.roundedRect(x, y, width, 2.6, 1.3, 1.3, "F"); doc.setFillColor(...color); doc.roundedRect(x, y, Math.max(1.5, width * Math.min(100, Math.max(0, percent)) / 100), 2.6, 1.3, 1.3, "F"); }
function healthCard(doc: jsPDF, title: string, x: number, y: number, width: number, height: number) { doc.setFillColor(255, 255, 255); doc.setDrawColor(226, 232, 240); doc.roundedRect(x, y, width, height, 2.5, 2.5, "FD"); doc.setFont("helvetica", "bold"); doc.setTextColor(15, 23, 42); doc.setFontSize(8.5); doc.text(title, x + 5, y + 7); }
function drawFinancialHealth(doc: jsPDF, data: FinancialReportData) {
  const health = data.financialHealth; if (!health) return;
  const recap = data.transactionRecap; let page = 2; doc.addPage(); let y = 20; sectionTitle(doc, "Financial Health", y); y += 7;
  if (health.position) { const p = health.position; healthCard(doc, "Financial Position", PAGE.left, y, 178, 30); const cols = [["Beginning Net Worth", p.beginningNetWorth], ["Ending Net Worth", p.endingNetWorth], ["Change", p.change]] as const; cols.forEach(([label, value], index) => { const x = PAGE.left + 5 + index * 57; doc.setFont("helvetica", "normal"); doc.setTextColor(100, 116, 139); doc.setFontSize(6.5); doc.text(label, x, y + 14); doc.setFont("helvetica", "bold"); doc.setTextColor(index === 2 ? (value < 0 ? 220 : 5) : 15, index === 2 ? (value < 0 ? 38 : 150) : 23, index === 2 ? (value < 0 ? 38 : 105) : 42); doc.setFontSize(8); doc.text(`${index === 2 && value > 0 ? "+" : ""}${formatCurrency(value)}`, x, y + 21); }); doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(71, 85, 105); doc.text(p.changePercent === null ? "Change % —" : `${p.changePercent > 0 ? "+" : ""}${p.changePercent.toFixed(1)}%`, PAGE.right - 5, y + 26, { align: "right" }); if (p.investmentValuationLimited) { doc.setFont("helvetica", "normal"); doc.setTextColor(100, 116, 139); doc.setFontSize(5.5); doc.text("Investment valuation is ledger-based; historical market valuations are not available.", PAGE.left + 5, y + 26); } y += 37; }
  if (health.budgets.length) { sectionTitle(doc, "Budget Performance", y); y += 5; const budgets = [...health.budgets].sort((a, b) => b.utilizationPercent - a.utilizationPercent); for (const item of budgets) { if (y > 260) { footer(doc, recap, page); doc.addPage(); page += 1; y = 20; sectionTitle(doc, "Budget Performance", y); y += 5; } healthCard(doc, item.periodStart === health.budgets[0]?.periodStart ? item.name : `${item.name} · ${item.periodStart.slice(0, 7)}`, PAGE.left, y, 178, 14); const color: readonly [number, number, number] = item.status === "over_budget" ? [220, 38, 38] : item.status === "near_limit" ? [217, 119, 6] : EMERALD; progressBar(doc, PAGE.left + 5, y + 9, 105, item.utilizationPercent, color); doc.setFont("helvetica", "bold"); doc.setFontSize(6.5); doc.setTextColor(...color); doc.text(`${item.utilizationPercent.toFixed(0)}% · ${item.status === "on_track" ? "On Track" : item.status === "near_limit" ? "Near Limit" : "Over Budget"}`, PAGE.left + 114, y + 10.5); doc.setFont("helvetica", "normal"); doc.setTextColor(71, 85, 105); doc.setFontSize(5.8); doc.text(`${formatCurrency(item.spent)} / ${formatCurrency(item.budgeted)} · Remaining ${formatCurrency(item.remaining)}`, PAGE.left + 5, y + 13); y += 16; } y += 3; }
  if (health.goals.length) { if (y > 220) { footer(doc, recap, page); doc.addPage(); page += 1; y = 20; } sectionTitle(doc, "Goals Progress", y); y += 5; for (const goal of health.goals) { if (y > 260) { footer(doc, recap, page); doc.addPage(); page += 1; y = 20; sectionTitle(doc, "Goals Progress", y); y += 5; } healthCard(doc, goal.name, PAGE.left, y, 178, 14); progressBar(doc, PAGE.left + 5, y + 9, 105, goal.progressPercent); doc.setFont("helvetica", "bold"); doc.setTextColor(...EMERALD); doc.setFontSize(6.5); doc.text(`${goal.progressPercent.toFixed(0)}%`, PAGE.left + 114, y + 10.5); doc.setFont("helvetica", "normal"); doc.setTextColor(71, 85, 105); doc.setFontSize(5.8); doc.text(`${formatCurrency(goal.progress)} / ${formatCurrency(goal.target)} · Contributed ${formatCurrency(goal.contributedDuringPeriod)}`, PAGE.left + 5, y + 13); y += 16; } y += 3; }
  if (health.receivables.outstanding || health.receivables.collectedDuringPeriod || health.debts.outstanding || health.debts.paidDuringPeriod) { if (y > 235) { footer(doc, recap, page); doc.addPage(); page += 1; y = 20; } sectionTitle(doc, "Debt & Receivable", y); y += 5; [["Receivable", "Outstanding", health.receivables.outstanding, "Collected this period", health.receivables.collectedDuringPeriod], ["Debt", "Outstanding", health.debts.outstanding, "Paid this period", health.debts.paidDuringPeriod]].forEach(([title, aLabel, aValue, bLabel, bValue], index) => { const x = PAGE.left + index * 90.5; healthCard(doc, title as string, x, y, 87.5, 27); doc.setFont("helvetica", "normal"); doc.setTextColor(100, 116, 139); doc.setFontSize(6); doc.text(aLabel as string, x + 5, y + 13); doc.setFont("helvetica", "bold"); doc.setTextColor(15, 23, 42); doc.setFontSize(8); doc.text(formatCurrency(aValue as number), x + 5, y + 19); doc.setFont("helvetica", "normal"); doc.setTextColor(100, 116, 139); doc.setFontSize(5.8); doc.text(`${bLabel as string} ${formatCurrency(bValue as number)}`, x + 5, y + 24); }); }
  footer(doc, recap, page);
}
