...
export async function getFinancialReportData({ space, period }: { space: FinancialSpace; period: ReportPeriod }): Promise<FinancialReportData> {
  ...
  const spendingBudgets = budgetRows.filter((budget) => budget.targetType === "category" || budget.targetType === "envelope");
  const outsideGroups = new Map<string, ReportCategoryBreakdown>();
  completedEconomicExpenses.filter((transaction) => !spendingBudgets.some((budget) => budget.targetType === "envelope" ? budget.envelopeId === transaction.envelope_id : !transaction.envelope_id && budget.categoryId === transaction.category_id)).forEach((transaction) => {
    const key = transaction.category_id ?? "uncategorized";
    const current = outsideGroups.get(key) ?? { categoryId: transaction.category_id, categoryName: transaction.category?.name ?? "Uncategorized", amount: 0, transactionCount: 0, percentage: 0 };
    current.amount += toNumber(transaction.amount); current.transactionCount += 1; outsideGroups.set(key, current);
  });
  const unbudgetedSpending = Array.from(outsideGroups.values()).sort((a, b) => b.amount - a.amount);
  const unbudgeted = unbudgetedSpending.reduce((sum, item) => sum + item.amount, 0);
  ...
  unbudgetedSpending.forEach((item) => { item.percentage = eligibleSpending ? item.amount / eligibleSpending * 100 : 0; });
  ...
  const materiality = Math.max(50_000, eligibleSpending * 0.05);
  const planningInsights = [
    ...unbudgetedSpending.filter((item) => item.amount >= materiality).map((item) => ({ type: "outside_budget" as const, priority: item.amount, title: item.categoryName, message: `${item.categoryName} accounted for ${formatReportMoney(item.amount)} of unbudgeted spending across ${item.transactionCount} transaction${item.transactionCount === 1 ? "" : "s"} this month.`, suggestedAction: `If this spending is likely to recur, consider creating a ${item.categoryName} budget next month.` })),
    ...budgetItems.filter((item) => item.status === "over_budget" && item.variance >= materiality).map((item) => ({ type: "over_budget" as const, priority: item.variance, title: item.name, message: `${item.name} exceeded its budget by ${formatReportMoney(item.variance)}.`, suggestedAction: "Review whether this reflects normal spending before setting next month's allocation." })),
    ...budgetItems.filter((item) => item.status === "below_target" && Math.abs(item.variance) >= materiality).map((item) => ({ type: "target_shortfall" as const, priority: Math.abs(item.variance), title: item.name, message: `${item.name} reached ${item.progressPercent.toFixed(0)}% of its monthly target.`, suggestedAction: "When planning next month, consider whether sustainable unused spending allocation can help close the gap." })),
    ...budgetItems.filter((item) => item.status === "ahead_of_target" && item.variance >= materiality).map((item) => ({ type: "target_ahead" as const, priority: item.variance, title: item.name, message: `${item.name} reached ${item.progressPercent.toFixed(0)}% of its monthly target.`, suggestedAction: "If this pace is sustainable, consider retaining it in next month's plan." })),
    ...(eligibleSpending > 0 && unbudgeted / eligibleSpending >= 0.2 ? [{ type: "coverage" as const, priority: unbudgeted, title: "Budget coverage", message: `${(unbudgeted / eligibleSpending * 100).toFixed(0)}% of eligible spending occurred outside the monthly plan.`, suggestedAction: "Review the largest unbudgeted categories before creating next month's budget." }] : []),
