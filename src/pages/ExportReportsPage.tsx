...
export function ExportReportsPage() {
  ...
  const runExport = useCallback(async (kind: NonNullable<typeof exporting>) => {
    ...
    try {
      ...
      console.error("Failed to generate report export", exportFailure);
      setExportError(true);
    } finally { setExporting(null); }
  }, [activeSpace, data, exporting, period]);
  ...
  return <div className="mx-auto w-full space-y-5 pb-8"><PageHeader icon={Download} eyebrow={t("reports.eyebrow")} title={t("reports.title")} description={t("reports.description")} />
    ...
    <PageCard className="p-4 sm:p-5"><h2 className="font-extrabold text-slate-900">{t("reports.exportAs")}</h2><p className="mt-1 text-sm text-slate-600">{t("reports.exportDescription")}</p><div className="mt-5 grid gap-5 md:grid-cols-2"><div><p className="text-sm font-extrabold text-slate-800">{t("reports.financialReport")}</p><p className="mt-1 text-xs text-slate-600">{t("reports.financialPdfHint")}</p><div className="mt-3"><Button disabled={!data || Boolean(exporting)} isLoading={exporting === "financial-pdf"} variant="secondary" onClick={() => void runExport("financial-pdf")}><FileText size={16} />PDF</Button></div></div><div><p className="text-sm font-extrabold text-slate-800">{t("reports.transactionRecap")}</p><p className="mt-1 text-xs text-slate-600">{t("reports.recapExportHint")}</p><div className="mt-3 flex flex-wrap gap-2"><Button disabled={!data || Boolean(exporting)} isLoading={exporting === "recap-pdf"} variant="secondary" onClick={() => void runExport("recap-pdf")}><FileText size={16} />PDF</Button><Button disabled={!data || Boolean(exporting)} isLoading={exporting === "xlsx"} variant="secondary" onClick={() => void runExport("xlsx")}><FileSpreadsheet size={16} />Excel</Button><Button disabled={!data || Boolean(exporting)} isLoading={exporting === "csv"} variant="secondary" onClick={() => void runExport("csv")}><Download size={16} />CSV</Button></div></div></div>{exportError ? <p className="mt-4 text-sm font-bold text-kash-expense">{t("reports.exportError")}</p> : null}</PageCard>
  </div>;
}
