...
import type { FinancialReportData, ReportPeriod, ReportPeriodPreset, TransactionRecapData, TransactionRecapFilters } from "../types/reports";
...
export function ExportReportsPage() {
  ...
  const runExport = useCallback(async () => {
    ...
    try {
      ...
      else if (financialData) await exportFinancialReportPdf(financialData);
  ...
  }, [data, exporting, financialData, format, reportKind]);

  useEffect(() => {
    if (!reportReady || !activeSpace || !period) { setData(null); setFinancialData(null); setLoading(false); return; }
    const request = ++requestRef.current;
    setLoading(true); setError(false); setData(null); setFinancialData(null);
    const loader = reportKind === "financial" ? getFinancialReportData({ space: activeSpace, period }).then((result) => ({ recap: result.transactionRecap, financial: result })) : getTransactionRecapData({ space: activeSpace, period, filters: appliedFilters }).then((result) => ({ recap: result, financial: null }));
    void loader.then((result) => {
      if (request === requestRef.current && result.recap.space.id === activeSpace.id) { setData(result.recap); setFinancialData(result.financial); }
    }).catch(() => { if (request === requestRef.current) setError(true); }).finally(() => { if (request === requestRef.current) setLoading(false); });
    return () => { requestRef.current += 1; };
  }, [activeSpace, appliedFilters, period, reportKind, reportReady]);

  const ExportIcon = formatIcons[format];
  const disabled = !data || loading || Boolean(exporting) || (reportKind === "financial" && !financialData) || (reportKind === "recap" && data.transactions.length === 0);
  ...
  const budgetVsActual = financialData?.budgetVsActual;
  return <div className="w-full space-y-5 pb-8">
    ...
    {loading || (status === "authenticated" && spaceLoading) ? <PageCard className="p-6 text-sm font-bold text-slate-600">{t("reports.loading")}</PageCard> : error ? <PageCard><EmptyState icon={FileText} tone="expense" title={t("reports.errorTitle")} description={t("reports.errorDescription")} /></PageCard> : data ? <PageCard className="p-4 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-extrabold text-slate-900">{t("reports.reportSummary")}</p><p className="mt-1 text-sm text-slate-600">{data.period.label} · {data.space.name}</p></div><span className="rounded-full bg-kash-selected px-3 py-1 text-xs font-extrabold text-kash-emeraldDark">{selectedReportLabel} · {format.toUpperCase()}</span></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">{summaryMetrics.map(([label, value]) => <SummaryItem key={label} label={label} value={value} />)}</div>{reportKind === "financial" ? <><FinancialReportCharts data={data} labels={{ cashFlow: t("reports.cashFlowTrend"), category: t("reports.categoryBreakdown"), wallet: t("reports.cashOutByWallet"), income: isManaged ? t("reports.funding") : t("reports.income"), expense: isManaged ? t("reports.spending") : t("reports.expense"), net: isManaged ? t("reports.netFlow") : t("reports.netCashFlow"), noData: t("reports.chartEmpty") }} />{budgetVsActual ? <div className="mt-5 rounded-xl border border-slate-200 p-4"><h3 className="font-extrabold text-slate-900">{t("reports.budgetVsActual")}</h3><div className="mt-3 space-y-2">{[...budgetVsActual.spending, ...budgetVsActual.targets].slice(0, 6).map((item) => <div key={item.id} className="flex items-center justify-between gap-3 text-sm"><span className="min-w-0 truncate font-semibold text-slate-700">{item.name}</span><span className={item.isFavorable ? "font-extrabold text-kash-emeraldDark" : "font-extrabold text-kash-expense"}>{formatCurrency(item.actual)} / {formatCurrency(item.planned)} · {item.progressPercent.toFixed(0)}%</span></div>)}</div></div> : null}{health ? <div className="mt-5 grid gap-3 sm:grid-cols-3"><SummaryItem label={t("reports.financialPosition")} value={health.position ? formatCurrency(health.position.endingNetWorth) : "—"} /><SummaryItem label={t("reports.budgetHealth")} value={budgetVsActual ? t("reports.overBudgetCount", { count: budgetVsActual.spending.filter((item) => item.status === "over_budget").length }) : t("reports.noBudgets")} /><SummaryItem label={t("reports.debtReceivable")} value={`${formatCurrency(health.debts.outstanding)} / ${formatCurrency(health.receivables.outstanding)}`} /></div> : null}</> : null}{data.transactions.length === 0 ? <div className="mt-5 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">{t("reports.emptyExportHint")}</div> : null}<div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-slate-500">{reportKind === "financial" ? t("reports.financialPdfHint") : t("reports.recapExportHint")}</p><Button className="w-full sm:w-auto" disabled={disabled} isLoading={Boolean(exporting)} onClick={() => void runExport()}><ExportIcon size={17} />{downloadLabel}</Button></div>{exportError ? <p className="mt-4 text-sm font-bold text-kash-expense">{t("reports.exportError")}</p> : null}</PageCard> : null}
  </div>;
}
