...
export function ExportReportsPage() {
  ...
  return <div className="w-full space-y-5 pb-8">
    ...
    {loading || (status === "authenticated" && spaceLoading) ? <PageCard className="p-6 text-sm font-bold text-slate-600">{t("reports.loading")}</PageCard> : error ? <PageCard><EmptyState icon={FileText} tone="expense" title={t("reports.errorTitle")} description={t("reports.errorDescription")} /></PageCard> : data ? <PageCard className="p-4 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-extrabold text-slate-900">{t("reports.reportSummary")}</p><p className="mt-1 text-sm text-slate-600">{t("reports.summaryHint")}</p></div><span className="rounded-full bg-kash-selected px-3 py-1 text-xs font-extrabold text-kash-emeraldDark">{selectedReportLabel}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><SummaryItem label={t("reports.space")} value={data.space.name} /><SummaryItem label={t("reports.period")} value={data.period.label} /><SummaryItem label={t("reports.transactions")} value={String(data.summary.transactionCount)} /><SummaryItem label={t("reports.exportFormat")} value={format.toUpperCase()} /></div>{data.transactions.length === 0 ? <div className="mt-5 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">{t("reports.emptyExportHint")}</div> : null}<div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-slate-500">{reportKind === "financial" ? t("reports.financialPdfHint") : t("reports.recapExportHint")}</p><Button className="w-full sm:w-auto" disabled={disabled} isLoading={Boolean(exporting)} onClick={() => void runExport()}><ExportIcon size={17} />{downloadLabel}</Button></div>{exportError ? <p className="mt-4 text-sm font-bold text-kash-expense">{t("reports.exportError")}</p> : null}</PageCard> : null}
  </div>;
}

function SummaryItem({ label, value }: { label: string; value: string }) { return <div className="min-w-0 rounded-xl bg-slate-50 px-3 py-3"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 truncate text-sm font-extrabold text-slate-800" title={value}>{value}</p></div>; }
